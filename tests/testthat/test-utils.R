...
test_that("read_robust_general pushes all I/O boundary conditions", {
  ...
  res_tsv <- looplook:::read_robust_general(tmp_tsv, header = TRUE, row_name = 1, min_cols = 2)
  expect_s3_class(res_tsv, "data.frame")

  tmp_csv <- tempfile(fileext = ".csv")
  write.csv(data.frame(Col1 = 1:3, Col2 = 4:6, Col3 = 7:9), tmp_csv, row.names = FALSE)
  res_csv <- looplook:::read_robust_general(tmp_csv, header = TRUE, row_name = NULL, min_cols = 2)
  ...
  expect_error(looplook:::read_robust_general("fake_file.txt"), "not found")
  expect_error(looplook:::read_robust_general(tmp_tsv, min_cols = 100), "insufficient columns")
